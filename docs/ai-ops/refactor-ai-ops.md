...
> 日期：2026-09-08（v2：方案定案——JSON 状态文件设计，fetch 先行实现中；v3：2026-09-10 branch 落地；v4：2026-09-10 commit-msg 落地；v5：2026-09-10 commit-pr 落地）
> 状态：**方案已定案，fetch/autofix/branch/commit-msg/commit-pr 已在 tools/ai-ops-1.py 落地**；其余子命令（rerun/dispatch/commit）待后续迭代
...
- **已迭代范围**：fetch（2026-09-08）、autofix（2026-09-09）、branch（2026-09-10）、commit-msg（2026-09-10）、commit-pr（2026-09-10）已移植；其余命令逻辑不变、留待后续迭代
...
### 4.3 后续命令（已落地：branch 2026-09-10、commit-msg 2026-09-10、commit-pr 2026-09-10；剩余 rerun/dispatch/commit 待迭代）
...
- 已落地的 commit-pr（2026-09-10）：从 `.aiops-fix-info.json` 读
  org/project/branch/commit-msg-file；commit-msg 文件（`log/ai-ops/{commit-msg-file}`）
  首行 = PR 标题、第 3 行起 = 正文；流程 = `git add {org}/{project}/` →
  `git commit -F log/ai-ops/{commit-msg-file}` → `git push -u origin {branch}` →
  `gh pr create --base main --head {branch} --title <首行> --body <正文>`；
  **不再接收 project_dir/run_id 位置参数、不再用通配符搜索 commit-msg 文件**
  （路径直接来自状态文件，原 find_commit_msg_file 不移植）；安全约束：
  commit-msg 文件缺失 → 列出 log/ai-ops 候选 exit 1、状态文件缺 branch → exit 1、
  当前分支为 main/master → 拒绝、当前分支 ≠ 状态文件 branch → 拒绝（提示先跑 branch）；
  `--dry-run/-n` 仅打印 4 条命令，且**跳过全部分支校验**（可在 main/master 上直接预览，
  仍先校验 commit-msg 文件存在）
- 剩余命令（rerun/dispatch/commit）均改为：**从 `.aiops-fix-info.json` 读 org/project/runid/date/文件路径**，不再接收 project_dir/run_id 位置参数（autofix 的 --version 改为可选且允许为空：缺省取状态文件 versions 首个版本，versions 为空则默认空字符串）
- 全部支持 `--dry-run/-n`
