...
docker buildx build \
    ...
    --load \
    -t lcr.loongnix.cn/library/ruby:4.0.6 \
    .

# 推送（buildx 直接推送，无需先 load）
docker buildx build \
    --platform linux/loong64 \
    --push \
    -t lcr.loongnix.cn/library/ruby:4.0.6 \
    -t lcr.loongnix.cn/library/ruby:latest \
    .
```
...
run_update_script() {
    ...
    cd "$PROJECT_DIR/template" && ./update.sh "$version" && cd - > /dev/null
}
...
run_apply_templates() {
    ...
    [[ "$DRY_RUN" == "true" ]] && return 0
    cd "$PROJECT_DIR/template" && ./apply-templates.sh "$version" && cd - > /dev/null
}
